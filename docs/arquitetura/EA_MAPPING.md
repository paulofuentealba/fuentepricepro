...
6. **Identity, Trust & Platform Enablement** — Auth, i18n, monetização, admin. **Atualização (Consolidação de Feature Gates)**: O sistema de Feature Gates foi consolidado em uma **fonte única de verdade** no documento Firestore `config/featureGates`. O switch mestre de código (`DISABLE_PAYWALLS`) foi removido do código-fonte. Por decisão de produto durante a fase de construção, o sistema foi configurado como **aberto por padrão (fail-open)** (`DEFAULT_FEATURE_GATES` com flags abertas e `freeAssetLimit: Infinity`). Bloqueios granulares futuros serão ativados individualmente via Firestore sem necessidade de deploys de código. Badges de conta ("Pro" vs "Free") permanecem como indicativos visuais de assinatura do usuário.
