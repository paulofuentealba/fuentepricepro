...
| 5 | **F3 — cor de CTA inconsistente** (`--primary` vs. `emerald` hardcoded, ~22 arquivos) | ✅ Resolvido (12/08) | P2 | F1/F2 (commit `aa83d3b`) já haviam migrado 19 arquivos; auditoria F3 confirmou que os únicos `emerald-*` restantes em `src/` são `src/routes/index.tsx` (mockup decorativo da landing, já excluído do gate por `design-tokens.test.ts`, representa ganho/resultado) e `src/lib/classify.ts` (paleta categórica de badges de tipo de ativo ON/PN/UNIT/BDR/Fracionário — fora de `src/components`/`src/routes`, não coberto pelo gate, e ambiguamente nem marca nem resultado; deixado como está, ver relatório 72 para decisão do Paulo) |
