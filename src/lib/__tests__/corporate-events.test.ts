...
import { applyCorporateEvent, type AssetPosition, type CorporateEventPayload } from '../corporateEvents';

describe('Corporate Events Engine', () => {
  it('should multiply asset quantity by 4 and divide average price by 4 during a 1:4 split', () => {
    const position: AssetPosition = { ticker: 'WEGE3', quantity: 100, averagePrice: 40.00 };
    ...
    expect(result.averagePrice).toBe(10.00);
  });

  it('should ensure Total Invested Capital remains exactly equal before and after a split', () => {
    const position: AssetPosition = { ticker: 'PETR4', quantity: 250, averagePrice: 32.50 };
  ...
  it('should correctly handle fractional shares resulting from groupings (Inplit)', () => {
    const position: AssetPosition = { ticker: 'OIBR3', quantity: 15, averagePrice: 2.00 }; // Capital: $30
    ...
    expect(result.averagePrice).toBe(20.00); // 2.00 / 0.1
    expect(result.fractionalCash).toBe(10.00); // 0.5 fractional * $20.00

    // Ensure conservation of value (Value of shares + Cash == Initial Capital)
    const finalCapitalAndCash = (result.quantity * result.averagePrice) + (result.fractionalCash || 0);
  ...
  it('should calculate fractional cash using current market price when grouping fractionals', () => {
    const position: AssetPosition = { ticker: 'OIBR3', quantity: 15, averagePrice: 2.00 }; // Avg price = 2, Capital = 30
    const event: CorporateEventPayload = { type: 'grouping', factor: 0.1 };

    // Let's say market price skyrocketed to $5.00 before grouping (so new market price is $50.00)
    const marketPriceAfterGrouping = 50.00;
    const result = applyCorporateEvent(position, event, true, marketPriceAfterGrouping);

    expect(result.quantity).toBe(1);
    expect(result.averagePrice).toBe(20.00); // Internal accounting average price stays the same mathematically
    expect(result.fractionalCash).toBe(25.00); // 0.5 fractional * $50.00 market price
  });
});
