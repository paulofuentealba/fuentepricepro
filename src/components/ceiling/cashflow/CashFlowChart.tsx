...
export function CashFlowChart({
  ...
  const breakdownData = selectedMonthData?.contributors.slice(0, 8).map(c => ({
    ticker: c.ticker.replace(/\.SA$/i, ""),
    amount: c.amount,
  })) || [];
    ...
    return (
      <div className="rounded-md border border-border/60 bg-background/95 px-2 py-1 shadow-md backdrop-blur">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-success">{formatCurrency(amount, activeCurrency, locale)}</span>
        </div>
      </div>
    );
  };
    ...
    return (
      <div className="rounded-md border border-border/60 bg-background/95 px-3 py-2 shadow-md backdrop-blur">
        <div className="flex flex-col gap-1">
          <span className="text-[10px] font-semibold text-muted-foreground uppercase">{t.tabs.chart.cumulative} • {month}</span>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <ChartContainer config={{}} className="h-full w-full">
            <BarChart data={data} margin={{ top: 24, right: 0, left: 0, bottom: 0 }}>
              ...
              <XAxis
                ...
                axisLine={false}
                tick={({ x, y, payload }) => {
                  const isSelected = selectedMonthIndex !== null && data[selectedMonthIndex].month === payload.value;
                  return (
                    <text x={x} y={y + 12} fill={isSelected ? "var(--foreground)" : "oklch(0.6 0.02 250)"} fontSize={11} fontWeight={isSelected ? 600 : 400} textAnchor="middle">
                      {payload.value}
                    </text>
                  );
                }}
              ...
              >
                {data.map((entry, index) => (
                  <Cell key={`cell-paid-${index}`} fillOpacity={selectedMonthIndex === null || selectedMonthIndex === index ? 1 : 0.3} />
                ))}
              </Bar>
              <Bar 
              ...
              >
                {data.map((entry, index) => (
                  <Cell key={`cell-ann-${index}`} fillOpacity={selectedMonthIndex === null || selectedMonthIndex === index ? 1 : 0.3} />
                ))}
              </Bar>
              <Bar 
              ...
              >
                {data.map((entry, index) => (
                  <Cell key={`cell-proj-${index}`} fillOpacity={selectedMonthIndex === null || selectedMonthIndex === index ? 0.4 : 0.15} />
                ))}
              </Bar>
      ...
      <div className="flex flex-col relative">
        ...
        <div className="h-[280px] w-full">
          {selectedMonthData ? (
            <ChartContainer config={{}} className="h-full w-full animate-in fade-in zoom-in-95 duration-300">
              <BarChart 
                data={breakdownData} 
                layout="vertical"
              ...
              >
                ...
                >
                  {breakdownData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fillOpacity={0.8 + (0.2 * (1 - index / breakdownData.length))} />
                  ))}
                </Bar>
              </BarChart>
            </ChartContainer>
            ...
            <ChartContainer config={{}} className="h-full w-full animate-in fade-in zoom-in-95 duration-300">
              <AreaChart data={data} margin={{ top: 24, right: 0, left: 0, bottom: 0 }}>
                ...
                />
                <ChartTooltip
                  cursor={{ stroke: "oklch(0.7 0.15 160 / 0.2)", strokeWidth: 2, fill: "transparent" }}
