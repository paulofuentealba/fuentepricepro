...
import { Download, Upload, Database, FileType2 } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
    ...
    try {
        ...
        if (lines.length > 0) {
          ...
          parsed = lines.slice(1).map((line) => {
            ...
            if (typeof obj.paymentMonths === "string" && obj.paymentMonths.length > 0) {
              obj.paymentMonths = obj.paymentMonths
                .split("|")
                .map(Number)
      ...
      for (const rawItem of parsed) {
        ...
        if (ticker) {
          ...
          if (!type) {
            if (ticker.endsWith(".SA") || /^[A-Z]{4}\d+$/.test(ticker) || ticker.endsWith("11"))
              type = "STOCK_BR";
            else type = "STOCK_US";
          }
          ...
          const item: WatchlistItem = {
            ...
            currency:
              rawItem.currency ||
              (type.includes("BR") || type === "FII" || type === "FIAGRO" ? "BRL" : "USD"),
            ...
            indexer: rawItem.indexer || null,
            rate: parseSafeNumber(rawItem.rate),
            startDate: rawItem.startDate || null,
            maturityDate: rawItem.maturityDate || null,
    ...
    <div className="flex items-center">
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-56">
          <DropdownMenuItem onClick={() => setOpenUploader(true)} className="gap-2 cursor-pointer font-medium text-success focus:text-success focus:bg-success/10">
            <FileType2 className="h-4 w-4" />
            <span>{t.brokerNote.importTitle}</span>
          </DropdownMenuItem>
          <DropdownMenuSeparator />
