  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center gap-2">
        <span className="font-semibold text-foreground">{cleanTicker}</span>
        {type && (
      ...
      {(name || sector) && (
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          {name && <span className="line-clamp-1 max-w-[120px]" title={name}>{name}</span>}
          {name && sector && <span>•</span>}
          {sector && <Badge variant="outline" className="text-[10px] font-normal">{sector}</Badge>}
        </div>
      )}
    </div>
  );
...
export function PriceTag({ 
  ...
  if (value === null || value === undefined || isNaN(value)) return <span className={cn("text-muted-foreground", className)}>—</span>;
