  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className={`${CONTAINER} py-12 md:py-16`}>
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as GuideTabId)}>
          <div className="-mx-4 overflow-x-auto px-4 pb-2 sm:mx-0 sm:px-0">
            <TabsList className="h-auto flex-nowrap gap-1 bg-muted/60 p-1">
              {visibleTabs.map((tab) => (
                <TabsTrigger
                  key={tab.id}
                  value={tab.id}
                  className="flex items-center gap-1.5 whitespace-nowrap px-3 py-1.5 text-xs sm:text-sm"
                >
